...
const PROMPTS = [
  'I sell tea in Lucknow.',
  'I run a fruit stall in Mumbai.',
  'I have a flower shop in Mysuru.',
  'I sell street food in Delhi.',
];
...
export default function ChatPage() {
  ...
  const send = (text: string) => {
    ...
    const newAnswers = currentField ? { ...answers, [currentField]: text } : answers;
    ...
    setTimeout(() => {
      ...
      if (step < FLOW.length - 1) {
        ...
        setMessages((m) => [...m, {
          role: 'ai',
          text: `Done! I've built your digital business, ${newAnswers.ownerName || 'Vendor'}! Here's everything I generated:`,
          results: [
            { icon: User, label: 'Business Profile', to: '/dashboard/profile' },
            ...
            { icon: Landmark, label: '3 Govt Schemes Matched', to: '/dashboard/schemes' },
          ],
        }]);
        toast.success('Digital business generated!');
      }
    }, 900);
  };
        ...
        <div className="mx-auto max-w-3xl px-4 py-6 space-y-4">
          ...
          {!done && step === 0 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {PROMPTS.map((p) => (
                <button key={p} onClick={() => send(p)} className="rounded-full border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3 py-1.5 text-xs hover:border-primary-400 hover:text-primary-600 transition-colors">
                  {p}
                </button>
              ))}
            </div>
          ...
          {done && (
            <Card className="p-5 bg-primary-50/50 dark:bg-primary-900/10 border-primary-200 dark:border-primary-900/40">
              ...
              <p className="text-sm text-gray-600 dark:text-zinc-400 mb-4">Review everything in your dashboard. You can edit details, download your QR, share your website and apply for schemes anytime.</p>
      ...
      <div className="border-t border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <div className="mx-auto max-w-3xl px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            />
            <button className="rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-zinc-800 text-gray-500"><Mic className="h-5 w-5" /></button>
